AFRAME.registerComponent("gaussian_splatting", {
  ...
  init: function () {
    ...
    document.querySelector("a-scene").addEventListener("loaded", () => {
      ...
      const setTransparency = () => {
        ...
        if (pointCloudEntity) {
          pointCloudEntity.object3D.add(this.points);
        }
        this._centerPointsObj = this.points;
        console.log(8);
      };
      setTimeout(setTransparency, 5000);
    });
    ...
    scene.addEventListener("click", (event) => {
      ...
      for (let t = 0; t < rayLength; t += boxStep) {
        ...
        for (let i = 0; i < positions.length; i += 3) {
          ...
          // 点をboxサイズ分拡大した仮想boxで判定
          const pointBox3 = new THREE.Box3(
            localPos.clone().addScalar(-boxSize2 / 4),
            localPos.clone().addScalar(boxSize2 / 4)
          );
          // Box同士の重なり判定
          if (box3.intersectsBox(pointBox3)) {
            console.log("contains-in");
            console.log(i);
            count++;
            // 一致した点のboxも生成
            const pointBox = document.createElement("a-box");
            pointBox.setAttribute("width", boxSize);
            pointBox.setAttribute("height", boxSize2);
            pointBox.setAttribute("depth", boxSize2);
  ...
  pushDataBuffer: function (buffer, vertexCount) {
    // スプラット中心点リストを初期化（初回のみ）
    if (!this.centers) this.centers = [];
    ...
    for (let i = 0; i < vertexCount; i++) {
      ...
      adjustedCenter.y = -adjustedCenter.y; // Y軸反転
